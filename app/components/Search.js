...
function Search() {
  ...
  useEffect(() => {
    if (state.requestCount) {
      ...
      async function fetchResults() {
        try {
          const resp = await Axios.post("/search", { searchTerm: state.searchTerm }, { cancelToken: ourReq.token })
          setState(draft => {
  ...
  function handleInput(e) {
    ...
    }) //we must use curly braces to not return the body of the function
  }
      ...
      <div className="search-overlay-bottom">
        <div className="container container--narrow py-3">
          ...
          <div className={"live-search-results " + (state.show == "results" ? "live-search-results--visible" : "")}>
            {Boolean(state.results.length) && ( //Boolean is for not displaying '0' on the screen
